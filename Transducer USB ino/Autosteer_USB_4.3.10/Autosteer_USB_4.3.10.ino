...
/////////////////////////////////////////////
  ...
  int presTransducerSP = 0; // Pressure transducer SetPoint from AOG 0 is 0v 255 is 5v
  int transducerPressure = 0; // from transducer reading 0 to 6640 is 0 to 5v
...
void loop()
{
    ...
    */
    ...
    //get pressure transducer reading       
    

    if (aogSettings.ShaftEncoder = 1)
    {
      //if (aogSettings.SingleInputWAS)   //Single Input ADS
    //{
      transducerPressure = ads.readADC_SingleEnded(2);    //ADS1115 Single Mode 
      
      transducerPressure = (transducerPressure >> 2); //bit shift by 2  0 to 6640 is 0 to 5v
    //}    
    //else    //ADS1115 Differential Mode
    //{
    //  steeringPosition = ads.readADC_Differential_2_3(); //ADS1115 Differential Mode
    //       
    //  steeringPosition = (steeringPosition >> 2); //bit shift by 2  0 to 6640 is 0 to 5v
    //}
    
      if ( presTransducerSP*26 < transducerPressure){
        pulseCount++;
      }
      
      // double the count if pressure is double of set point
      if ( presTransducerSP*52 < transducerPressure){
        pulseCount++;
      }
    }
  ...
  */
  ...
  if (Serial.available() > 0 && !isDataFound && !isSettingFound && !isMachineFound && !isAogSettingsFound) 
  {
    ...
    if (header == 32766) isDataFound = true;        //Do we have a match? here to add tranducer setting
  ...
  if (Serial.available() > 7 && isDataFound)
  {
    ...
    Serial.read();
    presTransducerSP = Serial.read(); // a byte with the setpoint
    ...
    //////////////////////////////////////////////////////////////////////////////////////
      ...
      Serial.print(",");
      Serial.print(transducerPressure); //pressure transducer value
      Serial.println(",0");  
  ...
  if (Serial.available() > 7 && isSettingFound) // here to add the pressure transducer setting
  {
    byte checksum = 0;
    byte reed = 0;
